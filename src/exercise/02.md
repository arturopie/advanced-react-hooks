...
Hint: when getting a react-hooks/exhaustive-deps violation, check if I need to use an `useCallback`

Would a better solution be to defined the function outside of the component, and pass Pokemon name as one of the arguments?

The users of an API should not need to know they have to use `useCallback`, instead, the designer of the API should return a function that uses `useCallback` and the user would call that function.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=Advanced%20React%20Hooks%20%F0%9F%94%A5&e=02%3A%20useCallback%3A%20custom%20hooks&em=arturop%40nulogy.com).
